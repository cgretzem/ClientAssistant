...
class UltimateBravery:
    ...
    def randomize_runes(self):
        ...
        prim_tree = Rune(trees[0], self.data.runes_dict[trees[0]]["ID"], self.data.runes_dict[trees[0]]["image"])
        sec_tree = Rune(trees[1], self.data.runes_dict[trees[1]]["ID"], self.data.runes_dict[trees[1]]["image"])

        prim_perks = self.__randomize_prims(prim_tree.name, ["Slot1", "Slot2", "Slot3"])
        sec_perks = self.__randomize_secs(sec_tree.name, ["Slot1", "Slot2"])
